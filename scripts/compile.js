...
//Compile colors
let compileColorsSCSS = function () {
    let colorsList = utils.readJSON(paths.colors.list);
    let colorsValues = [];
    Object.keys(colorsList).forEach(function (colorName) {
        return Object.keys(colorsList[colorName]).forEach(function (colorIndex) {
            return colorsValues.push({
                "index": `${colorName}-${colorIndex}`,
                "value": colorsList[colorName][colorIndex]
            });
        });
    });
    let colorsContent = appendAutoGeneratedHeader([]); //Initialize icons content
    colorsContent.push("$list: (");
    colorsValues.forEach(function (color, index) {
        let sep = (index === colorsValues.length - 1) ? "" : ","; //Separator
        colorsContent.push(`    "${color.index}": ${color.value}${sep}`);
    });
    colorsContent.push(") !default;");
    return fs.writeFileSync(paths.colors.lib, colorsContent.join("\n"), "utf8");
};
